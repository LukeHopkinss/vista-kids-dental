...
export default function Header() {
  ...
  // NEW: which mobile section is open (by label)
            ...
            {nav.map((item) => {
              if ("children" in item) {
                ...
                return (
                  ...
                  >
                    {/* label links to parent; caret toggles submenu */}
                ...
                if ("children" in item) {
                  ...
                  return (
                    <div key={item.label} className="py-1">
                      ...
                      >
                        {item.label} <span aria-hidden>{isOpen ? "▴" : "▾"}</span>
                      </button>

                      {isOpen && (
                        <ul
                          id={`mobile-sub-${item.label}`}
                          className="mt-1 ml-3 border-l pl-3 space-y-1"
                        >
                          {item.children.map((c) => (
                            <li key={c.href}>
                              <Link
